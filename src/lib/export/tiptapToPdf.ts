...
export const PW = 215.9;
export const PH = 279.4;
export const M = 25.4;
export const CW = PW - 2 * M;
export const PARA_INDENT = 12.7; // 0.5 inch
export const BQ_INDENT = 19.05; // 0.75 inch
...
export type Token = Seg & { w: number };
...
export function fstyle(bold: boolean, italic: boolean): string {
...
export function lh(pt: number): number {
  return pt * LH * PT_MM;
}

export function drawHeader(state: State): void {
  const d = state.doc;
  d.setFontSize(8);
...
export function drawFooter(state: State): void {
  const d = state.doc;
  d.setFontSize(9);
  d.setFont(FONT, "normal");
  d.setTextColor(122, 111, 99); // mist
...
export function drawPageChrome(state: State): void {
  drawHeader(state);
  drawFooter(state);
}

export function guard(state: State, needed: number): void {
...
export function renderNode(state: State, node: TNode): void {
  const d = state.doc;
